			...
			<ul>
				...
				<li>
					<table width="98%" border="0">
						<s:form method="post" action="province_update" namespace="/" theme="simple" onsubmit="return checkForm();">
							<s:hidden name="province.provinceId"></s:hidden>
							<tr>
								<th width="10%">机构名称</th>
							...
							<tr>
								<th width="10%">上级单位</th>
								<th width="80%"><s:select list="%{listProvince}" name="province.parentUnit"></s:select></th>
							</tr>
  <tr><th></th></tr>
  <tr><th></th></tr>
  <tr><th></th></tr>
  ...
  <tr>
	  ...
	  <select name="province.privilege">
	  	<c:forEach items="${rivilegeMap}" var="rivilege">
	  		<option value="${rivilege.key}" <c:if test="${rivilege.key eq province.privilege}">selected="selected"</c:if>>${rivilege.value}</option>
	    </c:forEach>
	  </select>
	  </th>
  </tr>
