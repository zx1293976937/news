...
<script>
	...
	function lookCount(){
		...
		var url = "${rootPath}/news_getCount.action?news.type="+hiddenType;
		window.parent.showCount(url);
	 }
</script>
</head>
<body>
	...
	<div class="row">
		<div class="col-xs-12">
			<div class="alert alert-info">
				您所在的位置<b class="tip"></b><a href="info_queryAllInfo">首页</a><b class="tip"></b><a href="#">新闻信息发布列表</a>
			</div>
		</div>
	</div>
	<div class="row" style="height:10px"></div>
	<form action="anews_toNewsFront" method="post" class="form-inline" onsubmit="return checkSearchForm();">
			<input type="hidden" name="pageNow" value="${pageNow }"/>
			<input type="hidden" name="news.type" value="${news.type}" id="hiddenType"/>
	</form>
	<%-- <div class="row">
		<div class="col-xs-12">
			...
			<table class="table table-bordered table-condensed">
				...
				<s:if test="%{newsList.size()>3}">
							<s:bean name="org.apache.struts2.util.Counter" id="counter">
								...
								<s:param name="last" value="newsList.size()" />
								<s:iterator value="newsList">
									<tr>
										<td><s:url id="test" action="news_front">
												<s:param name="news.newsId" value="%{newsId}" />
												
											</s:url> <s:a href="%{test}">${newsTitle}</s:a></td>
											<td style="width:30%">${newsATime}</td>
									</tr>
								</s:iterator>
							</s:bean>
						</s:if>
				...
				<tfoot>
					...
					<td colspan="2" style="padding: 5px;">
						<mytag:paging urlAction="anews_toNewsFront" type="1" />
					</td>
					</tr>
				</tfoot>
			</table>
