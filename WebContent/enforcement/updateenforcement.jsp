...
<li></li>
  ...
  <s:form method="post" action="enforcement_update" namespace="/" theme="simple" onsubmit = "return checkForm();">
	<s:hidden label="行政执法队伍名称" name="enforcement.enforcementId"></s:hidden> 
  <tr>
  ...
  <tr>
	  <th width="10%">独立设置</th>
	  <th width="80%" >
	  <input type="radio" name="enforcement.enforcementProperty" value="是" <c:if test="${enforcement.enforcementProperty eq '是'}">checked="checked"</c:if>/>是
	  	<input type="radio" name="enforcement.enforcementProperty" value="否" <c:if test="${enforcement.enforcementProperty eq '否'}">checked="checked"</c:if>/>否
	  </th>
  </tr>
   <tr><th></th></tr>
   <tr><th></th></tr>
    ...
    <tr>
	  ...
	  <th width="80%" >
	  	<div id="sTime" class="input-group date form_date">
			<input name="enforcement.enforcementInchargeBirthday" class="form-control" value="${enforcement.enforcementInchargeBirthday}">
   ...
   <tr><th></th></tr>
    <tr>
	  ...
	  <th width="80%" >
	  	<input type="radio" name="enforcement.enforcementInchargeLN" value="是" <c:if test="${enforcement.enforcementInchargeLN eq '是'}">checked="checked"</c:if>/>是
	  	<input type="radio" name="enforcement.enforcementInchargeLN" value="否" <c:if test="${enforcement.enforcementInchargeLN eq '否'}">checked="checked"</c:if>/>否
	  </th>
  </tr>
<s:hidden label="上传时间" name="enforcement.enforcementUTime"></s:hidden> 
 <tr><th></th></tr>
   ...
   <tr>
	  ...
	  <th width="80%" > 
	  	<input type="radio" name="enforcement.enforcementFPTime" value="专职" <c:if test="${enforcement.enforcementFPTime eq '专职'}">checked="checked"</c:if>/>专职
	  	<input type="radio" name="enforcement.enforcementFPTime" value="兼职" <c:if test="${enforcement.enforcementFPTime eq '兼职'}">checked="checked"</c:if>/>兼职
	  </th>
  </tr>
    ...
    <%-- <tr>
	  ...
	  <th width="10%">上级单位</th>
	  <th width="80%" ><s:textfield name="enforcement.enforcementParentUnit"  cssClass="form-control"></s:textfield> </th>
  </tr> --%>
